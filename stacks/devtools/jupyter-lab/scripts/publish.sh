#!/bin/bash
# Publish script for Jupyter Lab
...
echo -e "${BLUE}🚀 Publishing Jupyter Lab v${JUPYTER_VERSION} to Docker Hub${NC}"
echo ""
...
docker buildx build \
  ...
  --tag ${IMAGE_NAME}:${JUPYTER_VERSION} \
  --tag ${IMAGE_NAME}:${JUPYTER_VERSION}-debian \
...
if docker pushrm ${IMAGE_NAME} -f ${CURRENT_DIR}/README.md --short "Production-ready Jupyter Lab - Interactive notebook development environment for data science and ML"; then
...
echo "  • ${IMAGE_NAME}:${JUPYTER_VERSION}"
echo "  • ${IMAGE_NAME}:${JUPYTER_VERSION}-debian"
echo "  • ${IMAGE_NAME}:latest"
echo "  • ${IMAGE_NAME}:latest-debian"
echo ""
echo -e "${BLUE}View at: https://hub.docker.com/r/zylarian/dockyard-jupyter-lab${NC}"
echo ""
