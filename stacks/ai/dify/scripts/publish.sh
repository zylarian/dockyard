...
echo -e "${BLUE}🚀 Publishing Dify API v${DIFY_VERSION} to Docker Hub${NC}"
echo ""
...
docker buildx build \
  ...
  --tag ${IMAGE_NAME}:${DIFY_VERSION} \
  --tag ${IMAGE_NAME}:${DIFY_VERSION}-debian \
...
if docker pushrm ${IMAGE_NAME} -f ${CURRENT_DIR}/README.md --short "Production-ready Dify API - Backend for Dify LLM App Development Platform"; then
...
echo "  • ${IMAGE_NAME}:${DIFY_VERSION}"
echo "  • ${IMAGE_NAME}:${DIFY_VERSION}-debian"
echo "  • ${IMAGE_NAME}:latest"
echo "  • ${IMAGE_NAME}:latest-debian"
echo ""
echo -e "${BLUE}View at: https://hub.docker.com/r/zylarian/dockyard-dify-api${NC}"
echo ""
