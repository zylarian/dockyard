...
echo -e "${BLUE}🚀 Publishing Flowise v${FLOWISE_VERSION} to Docker Hub${NC}"
echo ""
...
docker buildx build \
  --platform ${PLATFORMS} \
  --build-arg FLOWISE_VERSION=${FLOWISE_VERSION} \
  --tag ${IMAGE_NAME}:${FLOWISE_VERSION} \
  --tag ${IMAGE_NAME}:${FLOWISE_VERSION}-debian \
...
if docker pushrm ${IMAGE_NAME} -f ${CURRENT_DIR}/README.md --short "Production-ready Flowise - Drag & drop UI to build customized LLM flows"; then
...
echo "  • ${IMAGE_NAME}:${FLOWISE_VERSION}"
echo "  • ${IMAGE_NAME}:${FLOWISE_VERSION}-debian"
echo "  • ${IMAGE_NAME}:latest"
echo "  • ${IMAGE_NAME}:latest-debian"
echo ""
echo -e "${BLUE}View at: https://hub.docker.com/r/zylarian/dockyard-flowise${NC}"
echo ""
