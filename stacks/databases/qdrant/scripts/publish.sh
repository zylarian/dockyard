...
IMAGE_NAME="zylarian/dockyard-qdrant"
...
echo -e "${BLUE}🚀 Publishing QDrant v${QDRANT_VERSION} to Docker Hub${NC}"
echo ""
...
docker buildx build \
  --platform ${PLATFORMS} \
  --tag ${IMAGE_NAME}:${QDRANT_VERSION} \
  --tag ${IMAGE_NAME}:${QDRANT_VERSION}-debian \
...
if docker pushrm ${IMAGE_NAME} -f ${CURRENT_DIR}/README.md --short "Production-ready QDrant - Vector Database for the next generation of AI applications"; then
...
echo "  • ${IMAGE_NAME}:${QDRANT_VERSION}"
echo "  • ${IMAGE_NAME}:${QDRANT_VERSION}-debian"
echo "  • ${IMAGE_NAME}:latest"
echo "  • ${IMAGE_NAME}:latest-debian"
echo ""
echo -e "${BLUE}View at: https://hub.docker.com/r/zylarian/dockyard-qdrant${NC}"
echo ""
