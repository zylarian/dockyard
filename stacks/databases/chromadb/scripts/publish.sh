...
IMAGE_NAME="zylarian/dockyard-chromadb"
...
echo -e "${BLUE}🚀 Publishing ChromaDB v${CHROMADB_VERSION} to Docker Hub${NC}"
echo ""
...
docker buildx build \
  --platform ${PLATFORMS} \
  --tag ${IMAGE_NAME}:${CHROMADB_VERSION} \
  --tag ${IMAGE_NAME}:${CHROMADB_VERSION}-debian \
...
if docker pushrm ${IMAGE_NAME} -f ${CURRENT_DIR}/README.md --short "Production-ready ChromaDB - The AI-native open-source embedding database"; then
...
echo "  • ${IMAGE_NAME}:${CHROMADB_VERSION}"
echo "  • ${IMAGE_NAME}:${CHROMADB_VERSION}-debian"
echo "  • ${IMAGE_NAME}:latest"
echo "  • ${IMAGE_NAME}:latest-debian"
echo ""
echo -e "${BLUE}View at: https://hub.docker.com/r/zylarian/dockyard-chromadb${NC}"
echo ""
